...
```{r}
salmon_quant_directory_sy5y = "~/Documents/Github/tdp43_concentration/data/salmon_curves/"
metadata_dir = fread("~/Documents/Github/tdp43_concentration/data/metadata.csv")
...
###single gene plot
normed_counts_long %>%
  dplyr::filter(symbol == "TARDBP" | symbol == "STMN2" | symbol == "CYFIP2" | symbol == "INSR") %>%
...
```{r}
#spaghetti - fix
spaghetti_plot(big_data) ###fix - use the new script??
#spaghetti_plot_normal(big_data)
```

```{r}
#create input for GO - DE (do it also on DS)
carmm <- results_table_sy5y_curves_0.025
...
###GO on DS
carm_ds <- big_delta %>% filter(.id == "dox0025" & no_dox_mean_psi < 0.05 & mean_dpsi_per_lsv_junction > 0.1) %>%
